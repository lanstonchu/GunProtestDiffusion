...
We have the latitude, longitude and time of each data point. We put them together to form a space-time metric space. In mathematics, a metric space ![equation](https://latex.codecogs.com/gif.latex?%28M%2C%20d%29) is a space with distance measure ![equation](https://latex.codecogs.com/gif.latex?d), which can map each pair of points in ![equation](https://latex.codecogs.com/gif.latex?M) into a real number, that is,

![equation](https://latex.codecogs.com/gif.latex?d%3A%20M%20%5Ctimes%20M%20%5Crightarrow%20%5Cmathbb%7BR%7D)

where ![equation](https://latex.codecogs.com/gif.latex?d) fulfills [some requirements][12] of having geometric meaning. Therefore we can use a unified distance function ![equation](https://latex.codecogs.com/gif.latex?d) for both time and space. To visualize the space-time metric space, we can plot a 3D scatter plot by using `Plot3D` in Wolfram Language, together with a map of US which is imported by `GeoGraphics` at the respective coordinates range. We will then see a "vessel-shaped" U.S. with "clouds of dust" as her constituents, and the plot looks like a 3D nebula map in astrophysics.
...
As mentioned, by putting space and time in the metric space ![equation](https://latex.codecogs.com/gif.latex?%28M%2C%20d%29), we are now able to define a united distance function ![equation](https://latex.codecogs.com/gif.latex?d) to measure the protests occurrences in a time/space compounded sense.

One of the major considerations is the corresponding length of spatial distance compared to a specific period of time (namely ![equation](https://latex.codecogs.com/gif.latex?r), i.e. ![equation](https://latex.codecogs.com/gif.latex?N) degree of latitude v.s. one year). This would affect the scale between space and time in the distance function, as we will use Euclidean Distance in our subsequent study.

This ratio ![equation](https://latex.codecogs.com/gif.latex?r) needs to make sense in the rate of diffusion. In the [event history analysis][15] of sociology, there is a sociological hypothesis that as a protest occurred in one locality, people there could inform and encourage their acquaintances elsewhere, who in turn could be inspired to initiate protest (Andrews and Biggs, 2006). We would, therefore, look at the normal practices of people's traveling inside the continent in their daily life for the space-time ratio ![equation](https://latex.codecogs.com/gif.latex?r) justification.

It is normal for individual citizens to physically travel from county to the neighboring county within a single day, for meeting their acquaintances or transporting resources. It is, therefore, reasonable to use 33 miles (which is considered as the average distance between counties; justified in the file [0.2 - counties distance.nb][16]) per day as our ratio ![equation](https://latex.codecogs.com/gif.latex?r). In this study, we would use degree (i.e. latitude and longitude) instead of miles as the spatial measure, which would be much more efficient in converting between the geographical info in data and the distance for the subsequent quantitative analysis. And thus we would take ![equation](https://latex.codecogs.com/gif.latex?r) to be 175 degrees per year (justified in 0.2 - counties distance.nb again).
...
In each cluster, we would apply techniques in graph theory to construct the network between the data points (i.e. the protests), and try to figure out how the practices of protest "jump" from one space-time duple (i.e. the latitude-longitude-time triple) to another duple (i.e. the spread inside the cluster). In graph theory,  a [spanning tree][22] ![equation](https://latex.codecogs.com/gif.latex?T) of an undirected graph ![equation](https://latex.codecogs.com/gif.latex?G) is a subgraph that is a tree which includes all of the vertices of ![equation](https://latex.codecogs.com/gif.latex?G), with the minimum possible number of edges. We would use the functions `DelaunayMesh` and `FindSpanningTree` as below, where `ctsSelJ` is the j-th cluster:
...
Each edge of the tree refers to a "jump" from one protest to another protest. Thus the jump would reflect the temporal and spatial distance differences between the previous and the next protest directly linked to the former. We would then try to model the temporal and spatial jumps by observing the statistics of jumps. We would like to focus on the spatial factor as a whole, and thus would combine latitude and longitude jump on Euclidean sense by ![equation](https://latex.codecogs.com/gif.latex?%24%5CDelta%20s%20%3D%20%5Csqrt%7B%5CDelta%20Lat%5E%7B2%7D%20&plus;%20%5CDelta%20Long%5E%7B2%7D%7D) for further processing. From the shape of the probability density function (PDF), we would assume that the spatial jumps of each cluster follow the Exponential Distribution ![equation](https://latex.codecogs.com/gif.latex?Exp%28%5Clambda%29), whose PDF is given as below:-

![equation](https://latex.codecogs.com/gif.latex?f%28x%3B%20%5Clambda%29%3D%5Cbegin%7Bcases%7D%5Clambda%20e%5E%7B-%5Clambda%20x%7D%2C%20%5Cquad%20x%5Cgeq%200%2C%5C%5C%200%2C%20%5Cqquad%20%5Cquad%20x%3C0.%20%5Cend%7Bcases%7D)

Then we would obtain the ![equation](https://latex.codecogs.com/gif.latex?%5Clambda) of each cluster by [maximum likelihood estimation][24] (i.e. MLE; EstimatedDistribution in Wolfram Language) as below, where λ1 and λ2 are the spatial and temporal parameters of the respective exponential distributions:-
...
Based on the occurred protest and the jump model obtained above, we can, therefore, make a prediction on the next protests based on the occurred events. At any specific time point  ![equation](https://latex.codecogs.com/gif.latex?t_%201) (say, one day away from the latest data), we would have a "cross section" at ![equation](https://latex.codecogs.com/gif.latex?t_%201), which can be considered as a ![equation](https://latex.codecogs.com/gif.latex?t_%201)-surface, namely ![equation](https://latex.codecogs.com/gif.latex?L_%20%7Bt_%201%7D). For every location ![equation](https://latex.codecogs.com/gif.latex?%28lat_%20i%2Clong_%20i%2Ct_%201%29) on ![equation](https://latex.codecogs.com/gif.latex?L_%20%7Bt_%201%7D), there will be temporal and spatial distances from it to the a respective nearest protest point. We would then subsequently obtain the PDF for the entire ![equation](https://latex.codecogs.com/gif.latex?L_%20%7Bt_%201%7D). Below is our protest prediction in the first few days in July 2018, based on the data up to June 30, 2018. We can see that Florida is most likely to have gun protests in early July 2018.
